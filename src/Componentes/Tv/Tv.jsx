...
export default function Tv() {
 ...
 async function getTrending(pageNumber){
  let {data}= await axios.get(`https://api.themoviedb.org/3/trending/tv/day?api_key=bdd10d2b8f52bc0a5320d5c9d88bd1ff&page=${pageNumber}`)
  setTrendingTv(data.results);
  
    }
    async function search(e){
      if(e.target.value){
        let {data}= await axios.get(`https://api.themoviedb.org/3/search/tv?api_key=f1aca93e54807386df3f6972a5c33b50&language=en-US&page=1&include_adult=false&query=${e.target.value}`)
        setTrendingTv(data.results);
      }else{
        getTrending(1);
      }
  ...
  return (
    <>
     ...
     <input onChange={search} type="text" placeholder='Search....' className='form-control bg-dark text-light mb-3'/>
      {trendingTv.slice(0,18).map((movie , index)=> <MediaItem movie={movie} key={index}/>  )}
